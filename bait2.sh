...
#cd-hit -i tmp2.fna -o tmp2.fna -c 1.00 
...
getorf -sequence tmp3.fna -outseq tmp1.faa -table $g &>/dev/null
...
################################################################################################################ align and retrotranslate
	...
	if [ -s tmp5.faa ]; 
		...
		pal2nal.pl tmp7.faa tmp4.fna -codontable $g -output fasta > def.aln #&>/dev/null;
